...
<head>
    ...
    <style>
        ...
        img.preview { margin-bottom: 0.5rem; border-radius: 0.25rem; }
    </style>
</head>
<body>
        ...
        <main class="flex-1 p-6">
            <div class="form-card">
                ...
                <form action="{{ route('team.update', $teamMember->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <textarea name="description" class="input-field" rows="4" placeholder="Description">{{ old('description', $teamMember->description) }}</textarea>

                    @if($teamMember->photo)
                        <img src="{{ asset('storage/'.$teamMember->photo) }}" alt="{{ $teamMember->name }}" width="100" class="preview">
                    @endif
                    <input type="file" name="photo" class="input-field" accept="image/*">
                    <small class="text-muted">Upload a new photo to replace the old one.</small>
